...
export default {
    ...
    computed: {
        ...
        uLabelWidth() {
            ...
            return this.elLabelPosition == 'left'
                ? this.label === 'true' || this.label === ''
                    ? 'auto'
                    : this.$u.addUnit(this.elLabelWidth)
                : '100%';
        },
        showError() {
            return (type) => {
        ...
        elLabelWidth() {
            ...
            return this.labelWidth != 0 || this.labelWidth != ''
                ? this.labelWidth
                : this.parentData.labelWidth
                ? this.parentData.labelWidth
                : 90;
        },
        // label的样式
        elLabelStyle() {
            return Object.keys(this.labelStyle).length
                ? this.labelStyle
                : this.parentData.labelStyle
                ? this.parentData.labelStyle
                : {};
        },
        // label的位置，左侧或者上方
    ...
    methods: {
        ...
        validation(trigger, callback = () => {}) {
            // 检验之间，先获取需要校验的值
            this.fieldValue = this.getByPath(this.parent.model, this.prop);
