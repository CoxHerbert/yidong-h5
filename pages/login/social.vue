...
<script>
import { getTopUrl, getCallbackUrl, isEnvironmentOld } from '@/utils/utils.js';
...
export default {
    ...
    created() {
        this.isWeixin = isEnvironmentOld() == 'wechat_open' || isEnvironmentOld() == 'wechat_enterprise';
        if (this.isWeixin) {
            uni.showLoading({
                title: '登录中...',
            });
        ...
        async getOpenidAndUserinfo(data) {
            this.$u.api.loginBySocial(data).then((res) => {
                ...
                if (data.error_description) {
                    ...
                    if (data.user_id == 'null' && isEnvironmentOld() == 'wechat_open') {
