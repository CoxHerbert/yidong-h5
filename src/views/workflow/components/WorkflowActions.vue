...
export default {
  ...
  emits: ['examine', 'user-select', 'draft', 'rollback', 'terminate', 'withdraw'],
  setup(props, { emit }) {
    ...
    const buttonMap = computed(() =>
      Object.fromEntries((props.buttonList || []).map((btn) => [btn.buttonKey, btn]))
    );
